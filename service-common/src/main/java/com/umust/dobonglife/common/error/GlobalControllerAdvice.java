package com.umust.dobonglife.common.error;
...
public class GlobalControllerAdvice {
    ...
    public ResponseEntity<BaseErrorResponse> handleNoHandlerFound(NoHandlerFoundException e) {
        ...
        return ResponseEntity.status(API_NOT_FOUND.getStatus()).body(new BaseErrorResponse(API_NOT_FOUND));
    }
    ...
    public ResponseEntity<BaseErrorResponse> handleMethodNotAllowed(HttpRequestMethodNotSupportedException e) {
        ...
        return ResponseEntity.status(METHOD_NOT_ALLOWED.getStatus()).body(new BaseErrorResponse(METHOD_NOT_ALLOWED));
    }
    ...
    public ResponseEntity<BaseErrorResponse> handleValidation(MethodArgumentNotValidException e) {
        ...
        return ResponseEntity.status(BAD_REQUEST.getStatus()).body(new BaseErrorResponse(BAD_REQUEST, detail));
    }

    @ExceptionHandler(MissingServletRequestParameterException.class)
    public ResponseEntity<BaseErrorResponse> handleMissingParam(MissingServletRequestParameterException e) {
        return ResponseEntity.status(BAD_REQUEST.getStatus()).body(new BaseErrorResponse(BAD_REQUEST));
    }
    ...
    public ResponseEntity<BaseErrorResponse> handleBusiness(BusinessException e) {
        ...
        return ResponseEntity.status(code.getStatus()).body(new BaseErrorResponse(code));
    }

    @ExceptionHandler(Exception.class)
    public ResponseEntity<BaseErrorResponse> handleException(Exception e) {
        log.error("[UnhandledException]", e);
        return ResponseEntity.status(INTERNAL_SERVER_ERROR.getStatus()).body(new BaseErrorResponse(INTERNAL_SERVER_ERROR));
    }
}
