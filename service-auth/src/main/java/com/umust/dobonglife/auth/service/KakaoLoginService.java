...
public class KakaoLoginService {
    ...
    public AuthTokens login(String accessToken, String fcmToken) {
        ...
        OAuthUserResponse user = userServiceClient.findOrCreateOAuthUser(
                Provider.KAKAO.name(), socialUser.providerId(), socialUser.email(), socialUser.name()
        );
        ...
        return loginSuccessUseCase.issueLoginToken(user.id(), Provider.KAKAO, Role.valueOf(user.role()), user.name());
    }
}
