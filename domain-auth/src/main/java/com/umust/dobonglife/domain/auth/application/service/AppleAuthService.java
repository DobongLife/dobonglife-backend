...
public class AppleAuthService {
    ...
    public SocialUserInfo verify(String identityToken) {
        ...
        if (email == null || email.isBlank()) {
            throw new BusinessException(DomainErrorCode.APPLE_TOKEN_EMAIL_MISSING);
        }
    ...
    private String exchangeAuthorizationCodeForRefreshToken(String authorizationCode) {
        try {
            ...
            throw new BusinessException(DomainErrorCode.APPLE_TOKEN_EXCHANGE_FAILED);
    ...
    private ErrorCode mapAppleTokenError(String responseBody) {
        if (responseBody == null) {
            return DomainErrorCode.APPLE_TOKEN_EXCHANGE_FAILED;
        }
        if (responseBody.contains("invalid_grant")) {
            return DomainErrorCode.APPLE_TOKEN_EXCHANGE_INVALID_GRANT;
        }
        if (responseBody.contains("invalid_client")) {
            return DomainErrorCode.APPLE_TOKEN_EXCHANGE_INVALID_CLIENT;
        }
        return DomainErrorCode.APPLE_TOKEN_EXCHANGE_FAILED;
    }
    ...
    private String generateClientSecret() throws Exception {
        ...
        Date now = new Date();
        Date expiration = new Date(now.getTime() + 15777000000L);
    ...
    private JWTClaimsSet verifyIdentityToken(String identityToken) {
        try {
            ...
            try {
                ...
                throw new BusinessException(DomainErrorCode.APPLE_TOKEN_EXPIRED);
            } catch (BadJOSEException retryEx) {
                log.error("Apple Identity Token 서명 검증 실패: {}", retryEx.getMessage());
                throw new BusinessException(DomainErrorCode.APPLE_TOKEN_SIGNATURE_INVALID);
    ...
    private JWTClaimsSet processToken(String identityToken, JWKSet jwkSet) throws Exception {
        ...
        if (!APPLE_ISSUER.equals(claims.getIssuer())) {
            ...
            throw new BusinessException(DomainErrorCode.APPLE_TOKEN_ISSUER_MISMATCH);
        }
        ...
        if (claims.getAudience() == null || !claims.getAudience().contains(appleClientId)) {
            ...
            throw new BusinessException(DomainErrorCode.APPLE_TOKEN_AUDIENCE_MISMATCH);
        }
    ...
    private JWKSet forceRefreshAppleJwkSet() {
        ...
        try {
            ...
            throw new BusinessException(DomainErrorCode.APPLE_JWKS_FETCH_FAILED);
        } finally {
            jwkLock.unlock();
        }
    }
