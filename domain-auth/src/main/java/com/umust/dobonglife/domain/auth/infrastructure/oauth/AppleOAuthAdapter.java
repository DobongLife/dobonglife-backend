package com.umust.dobonglife.domain.auth.infrastructure.oauth;
...
public class AppleOAuthAdapter implements AppleOAuthPort {
    ...
    public SocialUserInfo verify(String identityToken) {
        ...
        if (email == null || email.isBlank()) {
            throw new BusinessException(AuthErrorCode.APPLE_TOKEN_EMAIL_MISSING);
        }
    ...
    private ErrorCode mapAppleTokenError(String responseBody) {
        if (responseBody == null) {
            return AuthErrorCode.APPLE_TOKEN_EXCHANGE_FAILED;
        }
        if (responseBody.contains("invalid_grant")) {
            return AuthErrorCode.APPLE_TOKEN_EXCHANGE_INVALID_GRANT;
        }
        if (responseBody.contains("invalid_client")) {
            return AuthErrorCode.APPLE_TOKEN_EXCHANGE_INVALID_CLIENT;
        }
        return AuthErrorCode.APPLE_TOKEN_EXCHANGE_FAILED;
    }
    ...
    private JWTClaimsSet verifyIdentityToken(String identityToken) {
        try {
            ...
            try {
                ...
                throw new BusinessException(AuthErrorCode.APPLE_TOKEN_EXPIRED);
            } catch (BadJOSEException retryEx) {
                log.error("Apple Identity Token 서명 검증 실패: {}", retryEx.getMessage());
                throw new BusinessException(AuthErrorCode.APPLE_TOKEN_SIGNATURE_INVALID);
    ...
    private JWTClaimsSet processToken(String identityToken, JWKSet jwkSet) throws Exception {
        ...
        if (!APPLE_ISSUER.equals(claims.getIssuer())) {
            ...
            throw new BusinessException(AuthErrorCode.APPLE_TOKEN_ISSUER_MISMATCH);
        }
        ...
        if (claims.getAudience() == null || !claims.getAudience().contains(appleClientId)) {
            ...
            throw new BusinessException(AuthErrorCode.APPLE_TOKEN_AUDIENCE_MISMATCH);
        }
    ...
    private JWKSet forceRefreshAppleJwkSet() {
        ...
        try {
            ...
            throw new BusinessException(AuthErrorCode.APPLE_JWKS_FETCH_FAILED);
        } finally {
            jwkLock.unlock();
        }
    }
